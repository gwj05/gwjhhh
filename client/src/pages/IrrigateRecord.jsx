...
const IRRI_METHODS = ['滴灌', '喷灌', '沟灌', '漫灌', '微喷', '其他']
...
const IrrigateRecord = () => {
  ...
  const submitForm = async () => {
    ...
    const vol = Number(form.water_volume)
    if (Number.isNaN(vol) || vol <= 0) return showToast('用水量必须大于0', 'error')
    if (!form.method) return showToast('请选择灌溉方式', 'error')
    try {
      setSubmitting(true)
  ...
  const handleDelete = async (r) => {
    if (!canDelete) return
    if (!window.confirm('确认删除该灌溉记录？')) return
    try {
  ...
  return (
    <div className="operation-page">
      <div className="op-header">
        <div>
          <h2>灌溉记录</h2>
          <div className="op-sub">记录用水量与灌溉方式，不产生农资出库；数据会出现在操作记录查询中。</div>
        </div>
        {canCreate ? (
      ...
      <div className="stats-row cols-2">
        <div className="stat-card">
          <div className="label">灌溉次数（当前筛选）</div>
          <div className="value">{stats.irrigate_count ?? total}</div>
        </div>
        <div className="stat-card">
          ...
          <div className="modal-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-grid">
              ...
              <input
                ...
                onChange={(e) => setForm((p) => ({ ...p, water_volume: e.target.value }))}
              />
              <label>灌溉方式</label>
              <select
                value={form.method}
